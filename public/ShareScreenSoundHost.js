class ShareScreenSound {
  ...
  async createMixedAudioTrack(screenAudioTrack) {
    try {
      ...
      if (this.webrtcManager.localStream) {
        const micAudioTrack = this.webrtcManager.localStream.getAudioTracks()[0];
      ...
      console.error('Error creating mixed audio track:', error);
      return null;
    }
  }
...
document.addEventListener('DOMContentLoaded', () => {
  // Wait for the host meeting to be initialized
  const initializeScreenSound = () => {
    if (window.hostMeetingInstance && window.hostMeetingInstance.webrtc) {
      window.shareScreenSound = new ShareScreenSound(window.hostMeetingInstance.webrtc);
      console.log('ShareScreenSound initialized');
    } else {
